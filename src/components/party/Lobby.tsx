...
export function PartyLobby({ onBack, socketRef, isConnected }: LobbyProps) {
  const { t } = useI18n();
  ...
  const handleCreate = (difficulty: string, rounds: number, roundTime: number) => {
    if (!isConnected || !socket) return;
    if (!isValid) { setError(t('party.enterNick')); return; }
    setError('');
    setSettings({ difficulty, rounds, roundTime });
    ...
    socket.emit('party:create', { playerName: playerName.trim(), difficulty, rounds, roundTime });
  };
  ...
  const handleJoin = (code: string) => {
    ...
    socket.emit('party:join', { roomCode: trimmed, playerName: playerName.trim() });
  };
  ...
  return (
    <div style={{ textAlign: 'center', maxWidth: '480px' }}>
      ...
      <div style={{
        ...
        <button
          onClick={() => handleCreate(settings.difficulty, settings.rounds, settings.roundTime)}
...
export function HostSettings({
  ...
  settings: { difficulty: string; rounds: number; roundTime: number };
  onChange: (s: { difficulty: string; rounds: number; roundTime: number }) => void;
  disabled: boolean;
