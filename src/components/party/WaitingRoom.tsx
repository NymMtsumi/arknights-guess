...
import { HostSettings } from './Lobby';
...
export function PartyWaitingRoom({ socket, isConnected }: WaitingRoomProps) {
  ...
  const allReady = nonHostPlayers.every(p => p.ready) && players.length >= 4;
  ...
  const handleSettingsChange = (s: { difficulty: string; rounds: number; roundTime: number }) => {
    setSettings(s);
    ...
    <div style={{ textAlign: 'center', maxWidth: '520px' }}>
      ...
      <div style={{
        ...
        <div style={{ fontWeight: 700, fontSize: '0.9rem', marginBottom: '8px' }}>
          {t('party.players')} ({players.length}/6)
        </div>
        {players.map(p => (
          <div
            key={p.id}
        ...
        {players.length < 4 && (
          <p style={{ fontSize: '0.8rem', color: 'var(--warning)', marginTop: '8px' }}>
            {t('party.minPlayers', { current: players.length, min: 4 })}
          </p>
        )}
      </div>
        ...
        {isHost && (
          <button
            ...
            style={{
              ...
              cursor: players.length >= 4 && allReady ? 'pointer' : 'default',
              background: players.length >= 4 && allReady ? 'var(--accent)' : 'var(--card-soft)',
              color: players.length >= 4 && allReady ? '#fff' : 'var(--text-light)',
              opacity: players.length >= 4 && allReady ? 1 : 0.6,
            }}
          >
            🚀 {t('party.startGame')}
          </button>
